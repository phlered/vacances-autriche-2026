...
# Convert markdown to HTML while preserving iframes
html_content = md_content

# Protect HTML content (iframes, HTML blocks)
protected = {}
counter = 0

# Protect iframes
for match in re.finditer(r'<iframe[^>]*>.*?</iframe>', html_content, re.DOTALL):
    key = f'___PROTECTED_{counter}___'
    protected[key] = match.group(0)
    html_content = html_content.replace(match.group(0), key, 1)
    counter += 1
